...
function Validation(vm, key, vList, mList, tempIsPassKey, isPassKey, msgKey) {
  // 初始化
  this.vm = vm;
  this.key = key;
  ...
  self.vm.$watch(self.key, (newVal)=>{
    ...
    for(var k in self.vList) {
      ...
      (function (promise, tempResolve, tempReject, tempPromise) {
        ...
        promise.done(() => {
          rule.action.call(self.vm, newVal, attrVal, next);
        });
        tempPromise.fail(()=> {
  ...
  Componenet.implement({
    init: function(){
      ...
      for(var key in model) {
        ...
        var validation = new Validation(this, key, vList, mList, tempIsPassKey, isPassKey, msgKey);
